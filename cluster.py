...
class Clustering:
    ...
    def calcChromosomesFit(self):
        ...
        for i in range(0, numOfInd):
            ...
            for j in range(kmax):
                ...
                clusters.append(Cluster(dim, point))
                #print("centroid",clusters[0].centroid)
                #print("x" ,clusters[j].centroid)
            
            

            #print("centroid",clusters[0].centroid)
            #print("x" ,clusters[1].centroid)
    ...
    def printIBest(self, iBest):
        ...
        for i in range(0, 87):
            if z[i] == 0:
                correct_answer += 1
        for i in range(87, 156):
            if z[i] == 1:
                correct_answer += 1
    
        accuracy = (correct_answer / 156) * 100
